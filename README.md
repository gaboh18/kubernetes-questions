...
- [Question 5 – Build Container Image with Podman and Save as Tarball](#question-5--build-container-image-with-podman-and-save-as-tarball)
  - [Solution](#solution-4)
    - [Option 1: Using Podman](#option-1-using-podman)
...
- [Question 14 – Create Ingress Resource](#question-14--create-ingress-resource)
...
- [Question 17 - Docker: build and export as OCI Archive](#question-17--build-and-export-OCI-archive)
...
2. Update Deployment `secret-api-deploy` to use the Secret via `valueFrom.secretKeyRef`
...
```bash
podman save -o /tmp/my-app.tar my-app:1.0
```
...
```bash
docker save -o /tmp/my-app.tar my-app:1.0
```
...
- Deployment `canary-main-app` with 5 replicas, labels `app=webapp, version=v1`
- Service `canary-service` with selector `app=webapp`

Your task:
1. Scale Deployment `canary-main-app` to 8 replicas (80% of 10 total)
2. Create a new Deployment `web-app-canary` with 2 replicas, labels `app=webapp, version=v2`
3. Both Deployments should be selected by `canary-service`
...
```bash
kubectl scale deploy canary-main-app --replicas=8 -n default
```
...
```bash
kubectl get deploy canary-main-app -n default -o yaml > /tmp/web-app-canary.yaml
```
...
```bash
kubectl get endpoints canary-service -n default
...
In namespace `default`, Deployment `readiness-api-deploy` exists with a container listening on port `8080`.
...
```bash
kubectl edit deploy readiness-api-deploy -n default
```
...
```bash
kubectl rollout status deploy readiness-api-deploy -n default
kubectl describe deploy readiness-api-deploy -n default
```
...
In namespace `default`, Deployment `security-context-app` exists without any security context.
...
```bash
kubectl edit deploy security-context-app -n default
```
...
```bash
kubectl rollout status deploy security-context-app -n default
```
...
Your task:
Update Service `web-svc` to correctly select Pods from Deployment `selector-fix-deploy`.
...
In namespace `default`, Deployment `nodeport-api-deploy` exists with Pods labeled `app=api` and container port `9090`.
...
- Backend Service `web-svc-ingress` on port `8080`
...
```bash
kubectl get ingress web-svc-ingress -n default
kubectl describe ingress web-svc-ingress -n default
```
...
```bash
kubectl apply -f /tmp/fix-ingress.yaml
...
```bash
cat /tmp/fix-ingress.yaml
vi /tmp/fix-ingress.yaml
```
...
<a id="question-17"></a>
## Question 17 – Build and Export as OCI Archive

In the directory /tmp/oci-lab, a Dockerfile has been provided.

Your task:
1. Build a container image named internal-tool:v1.2 using the provided Dockerfile.
2. Export this image specifically in OCI archive format.
3. Save the resulting archive to /tmp/internal-tool-oci.tar .
4. Ensure the archive is OCI compliant (the exam may check for the index.json or oci-layaout files inside the tar).
